...
import { ChevronRight, Phone, Sparkles, ShieldCheck, Waves, MapPin, Calendar, FileText, Loader2, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function HomeClient({ initialServices, initialProjects, initialReviews }: HomeClientProps) {
  // Contact Form states
  const [contactName, setContactName] = useState('');
  const [contactEmail, setContactEmail] = useState('');
  const [contactService, setContactService] = useState('Water Well Drilling');
  const [contactMessage, setContactMessage] = useState('');
  const [contactSubmitting, setContactSubmitting] = useState(false);
  const [contactSuccess, setContactSuccess] = useState(false);
  const [contactError, setContactError] = useState<string | null>(null);

  const activeServices = initialServices.length > 0
    ? initialServices.map(s => ({
        title: s.name,
        description: s.short_description,
        accent: 'View service details →',
        href: `/services/${s.slug}`
      }))
    : fallbackServices;

  const activeTestimonials = initialReviews.length > 0
    ? initialReviews.map(t => ({
        quote: t.comment,
        author: `${t.name}, Client (${t.rating} ★)`
      }))
        ...
        {initialProjects.length > 0 && (
          <section className="mt-24" id="projects">
            <motion.div
              ...
              className="mb-10 max-w-3xl"
            >
              <p className="text-sm uppercase tracking-[0.3em] text-sky-200/80">Success Cases</p>
              <h2 className="mt-4 text-4xl font-semibold text-white">Completed Projects</h2>
              <p className="mt-4 text-slate-300">Explore some of our real drilling and maintenance works completed with success.</p>
            </motion.div>
              ...
              {initialProjects.map((project, index) => (
                ...
                >
                  <Link href={`/projects/${project.slug}`} className="group block h-full">
                    <Card className="h-full overflow-hidden flex flex-col justify-between p-0 border border-white/10 bg-slate-950/60 hover:bg-slate-950/80 hover:-translate-y-2 hover:shadow-[0_0_25px_rgba(0,194,255,0.12)] transition duration-300">
                      <div className="relative h-48 w-full bg-slate-900 overflow-hidden">
                        ...
                        {project.location && (
                          <span className="absolute bottom-3 left-3 text-[10px] bg-slate-950/80 text-sky-300 border border-white/5 px-2.5 py-1 rounded-full font-mono font-medium backdrop-blur-sm shadow-panel">
                            {project.location}
                          </span>
                        )}
                      </div>
                      <div className="p-6 space-y-3 flex-1 flex flex-col justify-between">
                        ...
                        {project.completion_date && (
                          <div className="flex items-center gap-1 text-[10px] text-slate-500 font-mono mt-4 pt-3 border-t border-white/5">
                            <Calendar className="h-3 w-3" />
                            <span>{new Date(project.completion_date).toLocaleDateString('en-US', { dateStyle: 'medium' })}</span>
                          </div>
                        )}
                      </div>
                    </Card>
        ...
        <section className="mt-24 rounded-[2rem] border border-white/10 bg-slate-950/70 p-10 shadow-panel" id="contact">
          <div className="grid gap-10 lg:grid-cols-[1fr_0.95fr]">
            ...
            <div className="relative">
              {contactSuccess ? (
                <div className="h-full rounded-[1.75rem] border border-emerald-500/20 bg-slate-900/80 p-8 shadow-glow flex flex-col items-center justify-center text-center space-y-4 min-h-[350px]">
                  ...
                  <p className="text-slate-300 text-sm max-w-xs leading-relaxed">
                    Thank you. Your request has been successfully submitted. We will contact you as soon as possible.
                  </p>
                  <Button onClick={() => setContactSuccess(false)} variant="secondary" className="mt-4">
                    Send another request
                  </Button>
                </div>
              ) : (
                <form onSubmit={handleContactSubmit} className="grid gap-4 rounded-[1.75rem] border border-white/10 bg-slate-900/80 p-8 shadow-glow">
                  {contactError && (
                    <div className="flex items-start gap-2.5 rounded-xl border border-red-500/20 bg-red-500/10 p-3.5 text-xs text-red-200">
                  ...
                  <Button type="submit" disabled={contactSubmitting} className="w-full">
                    {contactSubmitting ? (
                      <>
                        <Loader2 className="h-4 w-4 animate-spin text-slate-950 mr-2" />
                        Sending...
                      </>
                    ) : (
                      'Send request'
